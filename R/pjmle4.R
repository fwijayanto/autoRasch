...
# jml.gpcm.dif3.est <- function(X, opts$fixed_par = c(), opts$fixed_theta = c(), opts$fixed_beta = c(), opts$fixed_gamma = c(), opts$fixed_delta = c(), gamma_penalized = TRUE, delta_penalized = TRUE, theta_penalized = TRUE, resp.info = c(), resp.th = c(0), opt.method = c("nlminb"), plot.ll = FALSE, opt.tuner = list(iter.max = 20000, eval.max = 30000, rel.tol = 1e-10, step.max = 0.000001), opt.plot = list(), psi = 0.0078, max.iter = 150, objtype = "", desc = NULL,THETA.PCOEFF = 0.05, GAMMA.PCOEFF = 50, SMALLGAMMA.COEFF = 0.000005, delta.PCOEFF = 10000, DELTAGAMMA.PCOEFF = 10000, eps = 0.0, random.init = FALSE, random.init.th = 1e-2, init.par = c(), hessian = FALSE,tracked = TRUE){

pjmle4 <- function(X, randomized = FALSE, random.init.th = 1e-3, mt_vek_init = c(), init_par = c(), ...){

  dset <- as.data.frame(X)                          ### makes sure that the dataset has a matrix format


  dotdotdot <- list(...)
  # print(dotdotdot$fixed_par)

  # print(dim(dotdotdot$groups_map))
  # print(dim(dset))
  ...
  # delta.PCOEFF <- delta.PCOEFF
  # DELTAGAMMA.PCOEFF <- DELTAGAMMA.PCOEFF
  # eps <- eps

  dataPrep4 <- data_prep4(dset = dset, fixed_par = opts$fixed_par, groups_map = opts$groups_map, mt_vek_init = mt_vek_init)

  # print(dataPrep4$mt_vek)

  ### Intializing the parameters ###
  if(randomized){
    theta <- runif(nrow(dataPrep4$dset),-1,1)*random.init.th
    beta <- runif(dataPrep4$allcat,-1,1)*random.init.th
    gamma <- runif(ncol(dataPrep4$dset),-1,1)*random.init.th
    delta <- runif((ncol(dataPrep4$dset)*ncol(dataPrep4$groups_map)),-1,1)*random.init.th
    deltagamma <- runif((ncol(dataPrep4$dset)*ncol(dataPrep4$groups_map)),-1,1)*random.init.th
  } else {
    theta <- rep(0,nrow(dataPrep4$dset))
    beta <- rep(0,dataPrep4$allcat)
    gamma <- rep(0,ncol(dataPrep4$dset)) #gpcm uses different gamma for each item
    delta <- rep(0,(ncol(dataPrep4$dset)*ncol(dataPrep4$groups_map)))
    deltagamma <- rep(0,(ncol(dataPrep4$dset)*ncol(dataPrep4$groups_map)))
  }


  ### setting the optimized parameter
  nlmPar <- c(theta,beta,gamma,delta,deltagamma)
  ...
  length_array <- c(length_theta,length_beta,length_gamma,length_delta,length_deltagamma)
  fullPar_arr <- c("theta","beta","gamma","delta","deltagamma")
  ...
  if(!identical(grep("deltagamma",opts$fixed_par), integer(0))){
    ...
    fixValue <- c(rep(0,(ncol(dset)*ncol(dataPrep4$groups_map))),fixValue)
  }
  ...
  if(!identical(grep("^gamma",opts$fixed_par), integer(0))){
    if(length(nlmPar) == length(length_array)){
      ...
      if(!is.null(opts$fixed_gamma)){
        fixValue <- c(opts$fixed_gamma, rep(0,(ncol(dset)*ncol(dataPrep4$groups_map))), fixValue)
      } else {
        fixValue <- c(rep(0,ncol(dset)), rep(0,(ncol(dset)*ncol(dataPrep4$groups_map))), fixValue)
      }
   } else {
  ...
  output <- list("X" = X, "mt_vek" = dataPrep4$mt_vek, "real_vek" = dataPrep4$na_catVec, "itemName" = nameCol,
  ...
  time.v <- system.time(minRes <- optim(nlmPar, loglik_fun, gr = NULL, hessian = opts$isHessian, dset = dataPrep4$dset,
                                        ...
                                        fixLength_arr = fixLength_arr, allcat = dataPrep4$allcat, dimResp = dataPrep4$dimResp, n_th = dataPrep4$n_th, XN = dataPrep4$XN, XNA = dataPrep4$XNA, #XREAL = dataPrep4$XREAL,
                                        groups_map = dataPrep4$groups_map, mt_vek = dataPrep4$mt_vek, mt_idx = dataPrep4$mt_id, fixed_par = opts$fixed_par, fixValue = fixValue,
                                        isPenalized_gamma = opts$isPenalized_gamma, isPenalized_theta = opts$isPenalized_theta,
                                        isPenalized_delta = opts$isPenalized_delta, method = "BFGS", control = opts$optim_control, tracked =  opts$isTracked))
  ...
  if(!identical(grep("^beta",estPar_arr), integer(0))){
    ...
    if(parNo == 1){
      output[["beta"]] <- est[c((1):(sum(estLength_array[1])))]#*dataPrep4$na_catVec
    } else {
      output[["beta"]] <- est[c((sum(estLength_array[1:(parNo-1)])+1):(sum(estLength_array[1:parNo])))]#*dataPrep4$na_catVec
    }
  }
...
data_prep4 <- function(dset, fixed_par, groups_map, mt_vek_init){
  ...
  # if((max(minCat <- apply(dset,2,min,na.rm =TRUE)) != 0)){  ### makes sure the response is started at 0
  #   dset <- dset - matrix(rep(minCat,nrow(dset)),nrow = nrow(dset),byrow = TRUE)
  # }

  ### Checking the DIF group setting
  if(length(which(c("delta") %in% fixed_par)) == 0){
  ...
  # xn.mat <- matrix(rep(mt_idx,nrow(dset)),nrow = nrow(dset), ncol = allcat, byrow = TRUE)
  # xna.mat <- matrix(1,nrow = nrow(dset), ncol = allcat)## NA position
  # xreal.mat <- matrix(rep(na_catVec,nrow(dset)),ncol = allcat, byrow = TRUE)
  ...
  for(rIdx in 1:dimResp[1]){
    for(cIdx in 1:dimResp[2]){
      if(is.na(dset[rIdx,cIdx])){
        ...
        # print(mt_vek[cIdx])
        # print(dset[rIdx,cIdx])
        # print(mt_vek[cIdx]-dset[rIdx,cIdx])
  ...
  # xn.mat[full.idx] <- NA
  # xna.mat[full.idx] <- NA
  # xn.mat <- xn.mat * xreal.mat
  # XREAL <- as.vector(t(xreal.mat))
  # XN <- as.vector(t(xn.mat))
  # XNA <- as.vector(t(xna.mat))
...
#                   lambda_delta = 15, fixed_par = c(),
#                   isPenalized_gamma = TRUE, isPenalized_theta = TRUE, groups_map = c(),
#                   isPenalized_delta = TRUE, optim_control = list(maxit = 1e+7, reltol = 1e-12, fnscale = 10), isTracked = TRUE,
#                   fixed_theta = c(), fixed_beta = c(), fixed_gamma = c(), fixed_delta = c()){
#
...
#
#
#   # dataPrep4 <- data_prep(dset = dset, fixed_par = fixed_par, groups_map = groups_map)
#
#   ### Intializing the parameters ###
#   # if(random.init){
#   #   theta <- runif(nrow(dataPrep4$dset),-1,1)*random.init.th
...
#   delta <- rep(0,(ncol(dset)*ncol(groups_map)))
...
#   }
#   if(!identical(grep("delta",fixed_par), integer(0))){
#     nlmPar <- nlmPar[-c((sum(length_array[1:3])+1):(sum(length_array[1:4])))]
#     estLength_array <- estLength_array[-c(4)]
#     if(!is.null(fixed_delta)){
#       fixValue <- c(fixed_delta,fixValue)
#     } else {
#       fixValue <- c(rep(0,(ncol(dset)*ncol(groups_map))),fixValue)
#     }
#   }
...
#   #   delta.PCOEFF = lambda_delta, DELTAGAMMA.PCOEFF = lambda_delta, eps = eps,
...
#   #   theta_penalized = isPenalized_theta, delta_penalized = isPenalized_delta,
#   #   method = "BFGS", control = optim_control, tracked = isTracked))
#
#   ### Maximizing the loglikelihood function ###
#   print("Starting to estimate...")
#   nameCol <- colnames(as.data.frame(X))
...
#                                         lambda_delta = lambda_delta, lambda_deltagamma = 1e+6, eps = 0,
...
#                                         isPenalized_delta = isPenalized_delta, method = "BFGS", control = optim_control,
#                                         tracked =  isTracked))
...
#   #                                       mt_vek = mt_vek, fixPar = fixed_par, fixVal = fixValue, gamma_penalized = isPenalized_gamma,
#   #                                       theta_penalized = isPenalized_theta, delta_penalized = isPenalized_delta,
#   #                                       method = "BFGS", control = optim_control, tracked = isTracked))#list(maxit = 10000, lmm = 10, reltol = 1e-15, abstol = 1e-9))) #factr = 1e9, pgtol = 0.2
#
#   est <- minRes$par
#   obj <- minRes$value
#   iter <- minRes$iterations
