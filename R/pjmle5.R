...
pjmle5 <- function(X, randomized = FALSE, random.init.th = 1e-4, init_par = c(), eps = 0, ...){
  ...
  if(randomized){
    ...
    beta <- runif(dataPrep$allcat,-1,1)*random.init.th
    gamma <- runif(ncol(dataPrep$dset),-1,1)*random.init.th
    # delta <- runif((ncol(dataPrep$dset)*ncol(dataPrep$groups_map)),-1,1)*random.init.th
  ...
  time.v <- system.time(minRes <- optim(nlmPar, loglik_fun, gr = grad_fun, hessian = opts$isHessian, dset = dataPrep$dset,
                                        lambda_theta = opts$lambda_theta, lambda_in = opts$lambda_in,lambda_out = opts$lambda_out, eps = eps,
                                        ...
                                        groups_map = dataPrep$groups_map, mt_vek = dataPrep$mt_vek, mt_idx = dataPrep$mt_id, fixed_par = opts$fixed_par, fixValue = fixValue,
...
data_prep5 <- function(dset, fixed_par, groups_map){
