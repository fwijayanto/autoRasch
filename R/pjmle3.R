...
pjmle3 <- function(X, randomized = FALSE, random.init.th = 1e-3, mt_vek_init = c(), init_par = c(), eps = 0, ...){
  ...
  dataPrep3 <- data_prep3(dset = dset, fixed_par = opts$fixed_par, groups_map = opts$groups_map, mt_vek_init = mt_vek_init)

  # print(dataPrep3$mt_vek)

  ### Intializing the parameters ###
  if(randomized){
    theta <- runif(nrow(dataPrep3$dset),-1,1)*random.init.th
    beta <- runif(dataPrep3$allcat,-1,1)*random.init.th
    gamma <- runif(ncol(dataPrep3$dset),-1,1)*random.init.th
    delta <- runif((ncol(dataPrep3$dset)*ncol(dataPrep3$groups_map)),-1,1)*random.init.th
    deltagamma <- runif((ncol(dataPrep3$dset)*ncol(dataPrep3$groups_map)),-1,1)*random.init.th
  } else {
    theta <- rep(0,nrow(dataPrep3$dset))
    beta <- rep(0,dataPrep3$allcat)
    gamma <- rep(0,ncol(dataPrep3$dset)) #gpcm uses different gamma for each item
    delta <- rep(0,(ncol(dataPrep3$dset)*ncol(dataPrep3$groups_map)))
    deltagamma <- rep(0,(ncol(dataPrep3$dset)*ncol(dataPrep3$groups_map)))
  }
  ...
  if(!identical(grep("deltagamma",opts$fixed_par), integer(0))){
    ...
    fixValue <- c(rep(0,(ncol(dset)*ncol(dataPrep3$groups_map))),fixValue)
  }
  ...
  if(!identical(grep("^gamma",opts$fixed_par), integer(0))){
    if(length(nlmPar) == length(length_array)){
      ...
      if(!is.null(opts$fixed_gamma)){
        fixValue <- c(opts$fixed_gamma, rep(0,(ncol(dset)*ncol(dataPrep3$groups_map))), fixValue)
      } else {
        fixValue <- c(rep(0,ncol(dset)), rep(0,(ncol(dset)*ncol(dataPrep3$groups_map))), fixValue)
      }
   } else {
  ...
  output <- list("X" = X, "mt_vek" = dataPrep3$mt_vek, "real_vek" = dataPrep3$na_catVec, "itemName" = nameCol,
  ...
  if(!is.null(dotdotdot$groups_map)){
    output[["groups_map"]] <- dataPrep3$groups_map
  }
  ...
  time.v <- system.time(minRes <- optim(nlmPar, loglik_fun, gr = grad_fun, hessian = opts$isHessian, dset = dataPrep3$dset, eps = eps,
                                        ...
                                        fixLength_arr = fixLength_arr, allcat = dataPrep3$allcat, dimResp = dataPrep3$dimResp, n_th = dataPrep3$n_th, XN = dataPrep3$XN, XNA = dataPrep3$XNA, #XREAL = dataPrep3$XREAL,
                                        groups_map = dataPrep3$groups_map, mt_vek = dataPrep3$mt_vek, mt_idx = dataPrep3$mt_id, fixed_par = opts$fixed_par, fixValue = fixValue,
  ...
  if(!identical(grep("^beta",estPar_arr), integer(0))){
    ...
    if(parNo == 1){
      output[["beta"]] <- est[c((1):(sum(estLength_array[1])))]#*dataPrep3$na_catVec
    } else {
      output[["beta"]] <- est[c((sum(estLength_array[1:(parNo-1)])+1):(sum(estLength_array[1:parNo])))]#*dataPrep3$na_catVec
    }
  }
...
data_prep3 <- function(dset, fixed_par, groups_map, mt_vek_init){
...
#
#
#   # dataPrep3 <- data_prep(dset = dset, fixed_par = fixed_par, groups_map = groups_map)
#
#   ### Intializing the parameters ###
#   # if(random.init){
#   #   theta <- runif(nrow(dataPrep3$dset),-1,1)*random.init.th
