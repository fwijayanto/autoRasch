...
pjmleRel <- function(X, init_par = c(), ...){
  ...
  dataPrep <- data_prep_Rel(dset = dset, fixed_par = opts$fixed_par, groups_map = opts$groups_map)

  ### Intializing the parameters ###
  # if(random.init){
  #   theta <- runif(dataPrep$dimResp[1],-1,1)*random.init.th
  #   beta <- runif(allcat,-1,1)*random.init.th
  #   gamma <- runif(dataPrep$dimResp[2],-1,1)*random.init.th
  #   delta <- runif((dataPrep$dimResp[2]*ncol(dataPrep$groups_map)),-1,1)*random.init.th
  #   deltagamma <- runif((dataPrep$dimResp[2]*ncol(dataPrep$groups_map)),-1,1)*random.init.th
  # } else {
    theta <- rep(0,dataPrep$dimResp[1])
    beta <- rep(0,dataPrep$allcat)
    gamma <- rep(0,dataPrep$dimResp[2]) #gpcm uses different gamma for each item
    delta <- rep(0,(dataPrep$dimResp[2]*ncol(dataPrep$groups_map)))
    deltagamma <- rep(0,(dataPrep$dimResp[2]*ncol(dataPrep$groups_map)))
  # }
  ...
  if(!identical(grep("deltagamma",opts$fixed_par), integer(0))){
    ...
    fixValue <- c(rep(0,(dataPrep$dimResp[2]*ncol(dataPrep$groups_map))),fixValue)
  }
  ...
  if(!identical(grep("^gamma",opts$fixed_par), integer(0))){
    if(length(nlmPar) == length(length_array)){
      ...
      if(!is.null(opts$fixed_gamma)){
        fixValue <- c(opts$fixed_gamma, rep(0,(dataPrep$dimResp[2]*ncol(dataPrep$groups_map))), fixValue)
      } else {
        fixValue <- c(rep(0,dataPrep$dimResp[2]), rep(0,(dataPrep$dimResp[2]*ncol(dataPrep$groups_map))), fixValue)
      }
   } else {
  ...
  # } else if(opt.method == "optim"){
    ...
    time.v <- system.time(minRes <- optim(nlmPar, loglik_fun_Rel, gr = grad_fun_Rel, hessian = opts$isHessian, dset = dataPrep$dset,
                                          ...
                                          fixLength_arr = fixLength_arr, allcat = dataPrep$allcat,dimResp = dataPrep$dimResp, n_th = dataPrep$n_th, XN = dataPrep$XN, XNA = dataPrep$XNA,
...
data_prep_Rel <- function(dset, fixed_par, groups_map){
...
#
#
#   # dataPrep <- data_prep_Rel(dset = dset, fixed_par = fixed_par, groups_map = groups_map)
#
#   ### Intializing the parameters ###
#   # if(random.init){
#   #   theta <- runif(dataPrep$dimResp[1],-1,1)*random.init.th
